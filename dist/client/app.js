...
function createDefaultState() {
  ...
  return {
    ...
    skillExtraPoints: 0,
    effects: [],
    inventory: { money: { X: 0, D: 0, L: 0 }, patrimonio: "", weapons: [], equipment: [], armorId: "", customArmors: [], shield: false, shieldId: "", shieldMode: "", selectedWeaponId: "" },
    ...
    combat: {
      actions: { standard: true, bonus: true, movement: true, reaction: true },
      log: [],
      activeSpells: [],
      defenseAdjustments: { ca: 0, block: 0, vigor: 0 },
      parry: { prepared: false, count: 0 },
      fissure: { points: 0, attuned: false, magicUsedThisRound: false, prepared: false },
    },
    talents: [],
    abilities: [],
...
function effectiveVigorInfo(source = state) {
  const automatic = activeForteBonuses(source).vigor;
  const manual = num(source.combat?.defenseAdjustments?.vigor, 0);
  return { automatic, manual, total: automatic + manual };
}

function bodyInfo(source = state) {
  const vigor = effectiveVigorInfo(source).total;
...
function skillModifiers(skillName, source = state) {
  ...
  const shield = selectedShield(source);
  for (const mod of shield?.skillMods ?? []) {
    if (normalizeSkillName(mod.skill) === normalized) {
      mods.push({ source: `Escudo: ${shield.name}`, value: num(mod.value, 0), detail: `${shield.name}: ${signedValue(mod.value)} em ${normalized}` });
    }
  }
...
function selectedShield(source = state) {
  const shieldId = source.inventory?.shieldId || (source.inventory?.shield ? "escudo-redondo" : "");
  return (DB.shields ?? []).find((shield) => shield.id === shieldId) ?? null;
}

function shieldWithActiveMode(source = state) {
  const shield = selectedShield(source);
  if (!shield) return null;
  if (shield.id !== "escudo-grande" || source.inventory?.shieldMode !== "formation") return shield;
  return {
    ...shield,
    ca: num(shield.ca, 0) + 10,
    block: Object.fromEntries(Object.entries(shield.block ?? {}).map(([type, value]) => [type, num(value, 0) + 2])),
  };
}
...
function armorPieces(source = state) {
  ...
  const shield = shieldWithActiveMode(source);
...
function shieldEquipCard() {
  const shield = selectedShield();
  const activeMode = state.inventory.shieldMode === "formation"
    ? " · formação ativa"
    : state.inventory.shieldMode === "firmed" ? " · firmado" : "";
  return `<button class="shield-equip-card ${shield ? "equipped" : ""}" type="button" data-action="open-shield-picker">
    <span class="shield-glyph" aria-hidden="true"></span>
    <span>
      <strong>${esc(shield?.name ?? "Sem escudo")}</strong>
      <small>${shield ? `CA +${shield.ca} · ${armorBlockLabel(shield)}${activeMode}` : "Clique para escolher"}</small>
    </span>
  </button>`;
}

function shieldModeControls() {
  const shield = selectedShield();
  if (!shield) return "";
  if (shield.id === "escudo-grande") {
    const active = state.inventory.shieldMode === "formation";
    return `<button class="switch shield-mode-switch ${active ? "on" : ""}" type="button" role="switch" aria-checked="${active}" data-action="toggle-shield-formation">
      <span class="switch-track"><span class="switch-knob"></span></span>
      <span><strong>Formação protegida</strong><small>${active ? "+10 CA e +2 Bloqueio na direção protegida" : "Ative quando houver outro Escudo Grande adjacente"}</small></span>
    </button>`;
  }
  if (shield.id === "escudo-torre") {
    const firmed = state.inventory.shieldMode === "firmed";
    return `<button class="${firmed ? "ghost" : "button"} shield-mode-button" type="button" data-action="firm-tower-shield">${firmed ? "Abandonar posição firmada" : "Firmar escudo · Ação de Movimento"}</button>`;
  }
  return "";
}
...
function effectiveVigorPanel() {
  const vigor = effectiveVigorInfo();
  return `<section class="panel combat-vigor-panel">
    <div class="section-title"><div><h2>Vigor Efetivo</h2><p class="muted small">Fortes ativas alimentam o valor automático usado no cálculo do Corpo.</p></div><button class="ghost" type="button" data-action="open-defense-adjust" data-defense="vigor">Personalizar</button></div>
    <div class="vigor-stat-grid">
      ${miniStat("Automático", signedValue(vigor.automatic))}
      ${miniStat("Personalização", signedValue(vigor.manual))}
      ${miniStat("Vigor final", signedValue(vigor.total))}
      ${miniStat("Corpo atual", bodyInfo().mod)}
    </div>
  </section>`;
}

function ensureParryState() {
  state.combat.parry ??= { prepared: false, count: 0 };
  return state.combat.parry;
}

function parryPanel() {
  const parry = ensureParryState();
  const nextPenalty = num(parry.count, 0) * 5;
  const broquel = selectedShield()?.id === "broquel";
  return `<section class="panel combat-parry-panel ${parry.prepared ? "prepared" : ""}">
    <div class="section-title"><div><h2>Aparar</h2><p class="muted small">Ação Bônus para preparar. Cada Aparar depois do primeiro recebe -5 cumulativo.</p></div><span class="tag ${parry.prepared ? "ok" : "warn"}">${parry.prepared ? "Preparado" : "Não preparado"}</span></div>
    <div class="parry-summary">
      ${miniStat("Aparadas na rodada", parry.count)}
      ${miniStat("Próxima penalidade", nextPenalty ? `-${nextPenalty}` : "0")}
      ${miniStat("Broquel", broquel ? "+10 corpo a corpo" : "Sem bônus")}
      <button class="${parry.prepared ? "button" : "ghost"}" type="button" data-action="open-parry">${parry.prepared ? "Rolar Aparar" : "Preparar Aparar"}</button>
    </div>
  </section>`;
}

function ensureFissureState() {
  state.combat.fissure ??= { points: 0, attuned: false, magicUsedThisRound: false, prepared: false };
  return state.combat.fissure;
}

function fissureCelestePanel() {
  const fissure = ensureFissureState();
  return `<section class="panel fissure-panel ${fissure.attuned ? "attuned" : ""}">
    <div class="section-title"><div><h2>Fissura Celeste</h2><p class="muted small">01 natural após usar magia, ou Extremo depois da primeira Fissura da cena.</p></div><span class="tag ${fissure.attuned ? "ok" : ""}">${fissure.attuned ? "Sintonia ativa" : "Ainda sem sintonia"}</span></div>
    <div class="fissure-layout">
      <div class="fissure-points" aria-label="${fissure.points} de 8 Pontos de Fissura">
        ${Array.from({ length: 8 }, (_, index) => `<i class="${index < fissure.points ? "filled" : ""}"></i>`).join("")}
        <strong>${fissure.points}/8 Pontos</strong>
      </div>
      <div class="fissure-status">
        <span class="tag ${fissure.magicUsedThisRound ? "ok" : "warn"}">${fissure.magicUsedThisRound ? "Magia usada nesta rodada" : "Magia ainda não usada"}</span>
        ${fissure.prepared ? `<span class="tag ok">Próximo acerto preparado</span>` : ""}
      </div>
...
function parrySkills() {
  return DB.skills.filter((skill) => /^Lutar\s*\(/i.test(skill.name));
}

function openParryModal() {
  const parry = ensureParryState();
  if (!parry.prepared) {
    openModal("Preparar Aparar", `<p>Gaste sua <strong>Ação Bônus</strong> para poder Aparar ataques nesta rodada. Cada tentativa depois da primeira recebe -5 cumulativo.</p>`, `<button class="button" type="button" data-action="prepare-parry">Gastar Ação Bônus</button><button class="ghost" type="button" data-action="close-modal">Cancelar</button>`);
    return;
  }
  const skills = parrySkills();
  const nextPenalty = num(parry.count, 0) * 5;
  const broquelBonus = selectedShield()?.id === "broquel" ? 10 : 0;
  openModal("Aparar ataque", `
    <div class="stack">
      <div class="field"><label>Perícia Lutar</label><select id="parrySkill">${skills.map((skill) => `<option value="${esc(skill.name)}">${esc(skill.name)} · ${skillFinal(skill.name)}</option>`).join("")}</select></div>
      <div class="grid three">
        ${miniStat("Tentativa", parry.count + 1)}
        ${miniStat("Penalidade", nextPenalty ? `-${nextPenalty}` : "0")}
        ${miniStat("Broquel", broquelBonus ? "+10" : "0")}
      </div>
      <p class="muted small">O bônus do Broquel é aplicado somente nesta ferramenta, destinada a ataques corpo a corpo.</p>
    </div>
  `, `<button class="button" type="button" data-action="roll-parry" data-mode="normal">Normal</button><button class="ghost" type="button" data-action="roll-parry" data-mode="adv">Vantagem</button><button class="ghost" type="button" data-action="roll-parry" data-mode="dis">Desvantagem</button><button class="ghost" type="button" data-action="close-modal">Fechar</button>`);
}

function prepareParry() {
  const parry = ensureParryState();
  if (parry.prepared) return true;
  if (!useAction("bonus", "Preparar Aparar", false)) return false;
  parry.prepared = true;
  parry.count = 0;
  state.combat.log.unshift("Aparar preparado para esta rodada.");
  closeModal();
  render();
  return true;
}

function rollParry(mode = "normal") {
  const parry = ensureParryState();
  if (!parry.prepared) return openParryModal();
  const skillName = $("#parrySkill")?.value;
  if (!parrySkills().some((skill) => skill.name === skillName)) return addError("LAT-PT-001", skillName || "Perícia de Aparar ausente.");
  const count = num(parry.count, 0);
  const penalty = count * 5;
  const broquelBonus = selectedShield()?.id === "broquel" ? 10 : 0;
  const target = skillFinal(skillName) + broquelBonus - penalty;
  const roll = rollD100(mode);
  const outcome = d100Outcome(roll.result, target);
  parry.count = count + 1;
  publishRollResult("parry", roll, { skillName, target, outcome });
  state.combat.log.unshift(`Aparar ${parry.count}: ${roll.result} contra ${target} (${outcome}); penalidade -${penalty}${broquelBonus ? ", Broquel +10" : ""}.`);
  openModal("Resultado de Aparar", `
    <div class="stat"><span class="muted">${esc(roll.label)}</span><strong>${roll.result}</strong><small>Alvo ${target}</small></div>
    <p><strong>${esc(outcome)}</strong></p>
    <p class="muted">A próxima tentativa terá penalidade de -${parry.count * 5}.</p>
  `, `<button class="button" type="button" data-action="open-parry">Aparar novamente</button><button class="ghost" type="button" data-action="close-modal">Fechar</button>`);
}

function fissureDieSides(points) {
  return ({ 1: 20, 2: 12, 3: 10, 4: 8, 5: 6, 6: 4, 7: 2 })[num(points, 0)] ?? null;
}

function markMagicUsedThisRound() {
  ensureFissureState().magicUsedThisRound = true;
}

function openFissureAttempt() {
  const fissure = ensureFissureState();
  if (!fissure.points || fissure.prepared) return false;
  const options = Array.from({ length: fissure.points }, (_, index) => index + 1).map((points) => {
    const sides = fissureDieSides(points);
    return `<option value="${points}">${points} ponto(s) · ${sides ? `1d${sides}` : "Automático"}</option>`;
  }).join("");
  openModal("Tentar Fissura Celeste", `
    <div class="fissure-attempt-form stack">
      <p>Escolha quantos Pontos de Fissura gastar e um número possível no dado. Os pontos serão consumidos mesmo se a tentativa falhar.</p>
      <div class="grid two">
        <div class="field"><label>Pontos gastos</label><select id="fissureSpend">${options}</select></div>
        <div class="field"><label>Número escolhido</label><input id="fissureChosenNumber" type="number" min="1" max="20" value="1" inputmode="numeric"></div>
      </div>
      <p id="fissureAttemptPreview" class="muted" aria-live="polite">Tentativa com 1d20.</p>
    </div>
  `, `<button class="button" type="button" data-action="roll-fissure-attempt">Gastar e rolar</button><button class="ghost" type="button" data-action="close-modal">Cancelar</button>`);
}

function updateFissureAttemptPreview() {
  const points = num($("#fissureSpend")?.value, 1);
  const sides = fissureDieSides(points);
  const input = $("#fissureChosenNumber");
  if (input) {
    input.disabled = !sides;
    input.max = String(sides ?? 1);
    if (num(input.value, 1) > num(sides, 1)) input.value = "1";
  }
  if ($("#fissureAttemptPreview")) $("#fissureAttemptPreview").textContent = sides
    ? `Tentativa com 1d${sides}. Escolha um número de 1 a ${sides}.`
    : "Com 8 pontos, a preparação é automática.";
}

function rollFissureAttempt() {
  const fissure = ensureFissureState();
  const spent = num($("#fissureSpend")?.value, 0);
  if (!Number.isInteger(spent) || spent < 1 || spent > fissure.points) return toast("Quantidade de Pontos de Fissura inválida.", "warn");
  const sides = fissureDieSides(spent);
  let success = false;
  let detail = "Preparação automática";
  if (sides) {
    const chosen = num($("#fissureChosenNumber")?.value, 0);
    if (!Number.isInteger(chosen) || chosen < 1 || chosen > sides) return toast(`Escolha um número de 1 a ${sides}.`, "warn");
    const roll = ROLL_ENGINE.rollSync(ROLLS.createDieRequest({ sides }));
    publishRollResult("fissure_attempt", roll, { target: chosen, outcome: roll.result === chosen ? "Preparada" : "Falha" });
    success = roll.result === chosen;
    detail = `1d${sides}: escolhido ${chosen}, resultado ${roll.result}`;
  } else {
    success = spent === 8;
  }
  fissure.points -= spent;
  fissure.prepared = success;
  state.combat.log.unshift(`Tentativa de Fissura (${spent} ponto(s)): ${detail} — ${success ? "próximo acerto preparado" : "falhou"}.`);
  openModal(success ? "Fissura preparada" : "Tentativa encerrada", `<div class="stat"><span class="muted">${esc(detail)}</span><strong>${success ? "Sucesso" : "Falha"}</strong><small>${fissure.points}/8 Pontos restantes</small></div>${success ? `<p>O próximo ataque que acertar terá seus dados de dano triplicados e recuperará 10 PM.</p>` : ""}`);
}
...
function tripledDamageFormula(formula = "") {
  return String(formula || "Dano do ataque").replace(/(\d+)d(\d+)/gi, (_, count, sides) => `${num(count, 0) * 3}d${sides}`);
}

function resolveFissureAttack(roll, outcome, hit) {
  const fissure = ensureFissureState();
  const wasAttuned = fissure.attuned;
  const natural = hit && roll.result === 1 && fissure.magicUsedThisRound;
  const extreme = outcome === "Extremo";
  const prepared = hit && fissure.prepared;
  const triggered = natural || (hit && wasAttuned && extreme) || prepared;
  if (extreme && fissure.points < 8) {
    fissure.points += 1;
    state.combat.log.unshift(`Ataque Extremo: +1 Ponto de Fissura (${fissure.points}/8).`);
  }
  if (!triggered) return null;
  if (prepared) fissure.prepared = false;
  fissure.attuned = true;
  const before = pmCurrent();
  state.resources.pmCurrent = clamp(before + 10, 0, maxPm());
  const recovered = state.resources.pmCurrent - before;
  const weapon = state.inventory.weapons.find((item) => item.id === state.inventory.selectedWeaponId) ?? state.inventory.weapons[0];
  const damage = tripledDamageFormula(weapon?.damage ?? "Dados do ataque ×3");
  const reason = natural ? "01 natural após uso de magia" : prepared ? "preparação por Pontos de Fissura" : "Extremo em sintonia";
  state.combat.log.unshift(`Fissura Celeste: ${reason}. +${recovered} PM e dados de dano triplicados (${damage}).`);
  return { recovered, damage, reason };
}
...
function shieldPickerCard(shield) {
  const selected = selectedShield()?.id === shield.id;
  const price = `${num(shield.price?.D, 0)} D · ${num(shield.price?.C, 0)} C`;
  return `<article class="shield-picker-card ${selected ? "selected" : ""}">
    <header><span class="shield-glyph" aria-hidden="true"></span><div><strong>${esc(shield.name)}</strong><small>${esc(price)} · ${esc(shield.weight)}</small></div></header>
    <div class="grid three">
      ${miniStat("Bônus CA", `+${shield.ca}`)}
      ${miniStat("Bloqueio", armorBlockLabel(shield))}
      ${miniStat("Peso", shield.weight)}
    </div>
    <p>${esc(shield.property)}</p>
    <button class="${selected ? "button" : "ghost"}" type="button" data-action="select-shield" data-shield-id="${esc(shield.id)}">${selected ? "Selecionado" : "Selecionar"}</button>
  </article>`;
}

function openShieldPicker() {
  openModal("Escudos", `
    <div class="armor-picker-actions"><button class="ghost" type="button" data-action="select-shield" data-shield-id="">Sem escudo</button></div>
    <div class="shield-picker-grid">${(DB.shields ?? []).map(shieldPickerCard).join("") || `<div class="empty">Nenhum escudo cadastrado.</div>`}</div>
  `, `<button class="ghost" type="button" data-action="close-modal">Fechar</button>`);
}

function selectShield(id = "") {
  state.inventory.shieldId = id;
  state.inventory.shield = Boolean(id);
  state.inventory.shieldMode = "";
  if (id) state.combat.log.unshift(`${selectedShield()?.name ?? "Escudo"} equipado.`);
  else state.combat.log.unshift("Escudo removido.");
  closeModal();
  render();
}

function toggleShieldFormation() {
  if (selectedShield()?.id !== "escudo-grande") return false;
  const active = state.inventory.shieldMode === "formation";
  state.inventory.shieldMode = active ? "" : "formation";
  state.combat.log.unshift(active ? "Formação do Escudo Grande encerrada." : "Formação do Escudo Grande ativada: bônus direcionais aplicados.");
  render();
  return true;
}

function firmTowerShield() {
  if (selectedShield()?.id !== "escudo-torre") return false;
  if (state.inventory.shieldMode === "firmed") {
    state.inventory.shieldMode = "";
    state.combat.log.unshift("Escudo-Torre deixou a posição firmada.");
    render();
    return true;
  }
  if (!useAction("movement", "Firmar Escudo-Torre", false)) return false;
  state.inventory.shieldMode = "firmed";
  state.combat.log.unshift("Escudo-Torre firmado: Cobertura de 2/3 ativa até se deslocar ou abandonar a posição.");
  render();
  return true;
}
...
function ensureDefenseAdjustments() {
  if (!state.combat.defenseAdjustments) state.combat.defenseAdjustments = { ca: 0, block: 0, vigor: 0 };
  if (!Object.hasOwn(state.combat.defenseAdjustments, "vigor")) state.combat.defenseAdjustments.vigor = 0;
...
function openDefenseAdjustmentModal(kind = "ca") {
  const safeKind = ["block", "vigor"].includes(kind) ? kind : "ca";
  const ca = caBreakdown();
  const isBlock = safeKind === "block";
  const isVigor = safeKind === "vigor";
  const vigor = effectiveVigorInfo();
  const title = isBlock ? "Ajustar Bloqueio" : isVigor ? "Personalizar Vigor Efetivo" : "Ajustar CA";
  const calculated = isBlock ? blockText(ca.calculatedBlock) : isVigor ? vigor.automatic : ca.calculatedTotal;
  const finalValue = isBlock ? blockText(ca.block) : isVigor ? vigor.total : ca.total;
  const adjustment = isBlock ? ca.adjustments.block : isVigor ? vigor.manual : ca.adjustments.ca;
  openModal(title, `
    <div class="stack">
      <div class="grid three">
      ...
      <p class="muted">${isVigor ? "A personalização é somada às Magias Fortes ativas e atualiza automaticamente Corpo e Bloqueio." : "Este ajuste soma uma camada manual de combate e não altera atributos, armaduras, talentos, magias ou efeitos temporários."}</p>
    </div>
  `, `
...
function adjustDefense(kind, delta) {
  ...
  const label = safeKind === "block" ? "Bloqueio" : safeKind === "vigor" ? "Vigor Efetivo" : "CA";
  state.combat.log.unshift(`${label} manual ${signedValue(delta)}: ajuste ${signedValue(adjustments[safeKind])}.`);
  render();
...
function resetDefenseAdjustment(kind) {
  ...
  const adjustments = ensureDefenseAdjustments();
  const label = safeKind === "block" ? "Bloqueio" : safeKind === "vigor" ? "Vigor Efetivo" : "CA";
  if (num(adjustments[safeKind], 0)) state.combat.log.unshift(`${label} manual zerado.`);
...
function activateSpellItem(item, actionCost, options = {}) {
  ...
  const pmCost = Object.hasOwn(options, "pmCost")
    ? magicCostAfterCore(Math.max(0, num(options.pmCost, 0))) + magicPreparedExtraCost(item)
    : magicTotalPmCost(item);
...
function openFinaActivationModal(spellId) {
  const item = knownSpells().find((spell) => spell.id === spellId) ?? knownSpells().find((spell) => spell.type === "Fina");
  if (!item) return addError("LAT-MAG-002", "Fina");
  const normalAction = currentLevelEntry(item)?.activationAction ?? "bonus";
  const normalCost = magicTotalPmCost(item);
  const freeCost = magicCostAfterCore(5) + magicPreparedExtraCost(item);
  openModal("Ativar Magia Fina", `
    <p>A partir do N6, o Imbuimento de Fina pode ser realizado como Ação Livre por 5 PM.</p>
    <div class="grid two">
      <div class="card"><strong>Uso do nível atual</strong><p>${esc(actionCostLabel(normalAction))} · ${Number.isFinite(normalCost) ? `${normalCost} PM` : "custo manual"}</p></div>
      <div class="card"><strong>Imbuimento avançado</strong><p>Ação Livre · ${freeCost} PM</p></div>
    </div>
  `, `<button class="ghost" type="button" data-action="activate-fina" data-spell-id="${esc(item.id)}" data-mode="normal">Uso normal</button><button class="button" type="button" data-action="activate-fina" data-spell-id="${esc(item.id)}" data-mode="free">Imbuir · Ação Livre</button><button class="ghost" type="button" data-action="close-modal">Cancelar</button>`);
}

function activateFina(spellId, mode) {
  const item = knownSpells().find((spell) => spell.id === spellId) ?? knownSpells().find((spell) => spell.type === "Fina");
  if (!item) return addError("LAT-MAG-002", "Fina");
  closeModal();
  if (mode === "free") return activateSpellItem(item, "free", { pmCost: 5 });
  const normalAction = currentLevelEntry(item)?.activationAction ?? "bonus";
  return activateSpellItem(item, normalAction);
}
...
function completePassTurn() {
  ...
  state.combat.parry = { prepared: false, count: 0 };
  ensureFissureState().magicUsedThisRound = false;
...
function finishCombat() {
  ...
  state.combat.actions = { standard: true, bonus: true, movement: true, reaction: true };
  state.combat.defenseAdjustments = { ca: 0, block: 0, vigor: 0 };
  state.combat.parry = { prepared: false, count: 0 };
  state.combat.fissure = { points: 0, attuned: false, magicUsedThisRound: false, prepared: false };
  state.inventory.shieldMode = "";
  state.magicCore.preparedBoost = "";
...
function handleClick(event) {
  ...
  const actions = {
    ...
    "open-parry": openParryModal,
    "prepare-parry": prepareParry,
    "roll-parry": () => rollParry(button.dataset.mode),
    "open-fissure-attempt": openFissureAttempt,
    ...
    "activate-fina": () => activateFina(button.dataset.spellId, button.dataset.mode),
    ...
    "open-shield-picker": openShieldPicker,
    "select-shield": () => selectShield(button.dataset.shieldId ?? ""),
    "toggle-shield": () => selectShield(selectedShield() ? "" : "escudo-redondo"),
    "toggle-shield-formation": toggleShieldFormation,
    "firm-tower-shield": firmTowerShield,
...
function rollCombatTest(mode) {
  ...
  const fissure = resolveFissureAttack(roll, outcome, hit);
  ...
  openModal(`Teste: ${skill.label}`, `
    <div class="combat-roll-result stack">
      ...
      ${fissure ? `<div class="fissure-result"><strong>Fissura Celeste</strong><span>+${fissure.recovered} PM · dano ${esc(fissure.damage)}</span><small>${esc(fissure.reason)}</small></div>` : ""}
...
function activateMagic(type, spellId = "") {
  ...
  if (type === "Fina" && num(item?.level, 0) >= 6) return openFinaActivationModal(item?.id ?? spellId);
  ...
  return activateSpellItem(item, ["full", "standard", "bonus", "movement", "reaction", "free"].includes(structuredAction) ? structuredAction : (map[type] ?? "bonus"));
}

function useAction(cost, label, rerender = true) {
  if (cost === "free") {
    state.combat.log.unshift(`${label}: Ação livre usada.`);
    if (rerender) render();
    return true;
  }
  ...
  if (state.inventory.shieldMode === "firmed" && need.includes("movement") && label !== "Firmar Escudo-Torre") {
    state.inventory.shieldMode = "";
    state.combat.log.unshift("Cobertura do Escudo-Torre encerrada pelo deslocamento.");
  }
