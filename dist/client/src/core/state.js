(function initLatioState(root, factory) {
  ...
  function normalizeDomain(state, appId, schemaVersion) {
    ...
    state.combat ||= Object.create(null);
    ...
    state.inventory.shieldId = String(state.inventory.shieldId || (state.inventory.shield ? "escudo-redondo" : ""));
    state.inventory.shield = Boolean(state.inventory.shieldId);
    state.inventory.shieldMode = ["formation", "firmed"].includes(state.inventory.shieldMode) ? state.inventory.shieldMode : "";
    if (state.inventory.shieldId !== "escudo-grande" && state.inventory.shieldMode === "formation") state.inventory.shieldMode = "";
    if (state.inventory.shieldId !== "escudo-torre" && state.inventory.shieldMode === "firmed") state.inventory.shieldMode = "";
    ...
    state.combat.defenseAdjustments.vigor = clampNumber(state.combat.defenseAdjustments.vigor, -999, 999, 0);
    state.combat.parry.prepared = Boolean(state.combat.parry.prepared);
    state.combat.parry.count = clampNumber(state.combat.parry.count, 0, 999, 0);
    state.combat.fissure.points = clampNumber(state.combat.fissure.points, 0, 8, 0);
