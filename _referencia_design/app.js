...
const ERROR_CATALOG = Object.fromEntries([
  ...
  ["LAT-PDF-001", ["Importação PDF", "ERRO", "Não foi possível ler o PDF importado.", "Arquivo inválido, corrompido ou sem texto extraível.", "Conferir se o arquivo é uma ficha válida ou preencher manualmente."]],
...
async function getPdfLibrary() {
  if (!pdfLibraryPromise) {
    ...
    pdfLibraryPromise = import(pdfUrl).then((module) => {
...
function createDefaultState() {
  ...
  return {
    ...
    ui: { activeTab: "resumo", printMode: false, extraMagicType: "Fina", extraMagicRegion: "Sem Região", showExtraMagic: false, lawCategory: "Ofensivo", lawId: "" },
    character: {
      name: "",
      age: "",
      gender: "",
    ...
    combat: { actions: { standard: true, bonus: true, movement: true, reaction: true }, log: [] },
    talents: [],
    abilities: [],
...
function getSpell(type, regionCode = state.character.regionCode, cultureId = state.character.cultureId) {
  if (regionCode && regionCode !== "Sem Região") {
    const region = getRegion(regionCode);
    const culture = cultureId ? getCulture(cultureId) : getCulture();
    const cultureRegionalMap = {
      C: {
        "nan-mir": "Edito Do Céu",
        "n-rdalir": "Rugido Do Fiorde",
        "egr-mir": "Raiz Da Terra",
      },
    };
    const cultureSpellName = cultureRegionalMap[regionCode]?.[culture?.id];
    if (cultureSpellName && type === "Etérea") {
      const culturalRegional = region?.magics.find((magic) => magic.name === cultureSpellName);
      if (culturalRegional) return { ...culturalRegional, baseType: type };
    }
...
function spellCostTotal(type, level) {
  let total = 0;
  for (let current = 1; current <= level; current += 1) total += aptitudeCost(type, current);
  return total;
}

function aptitudeTotal() {
  const level = Math.max(1, num(state.character.level, 1));
  return Math.floor(attr("POD") / 7) + (level * 3) + num(state.magic.extraAptitudes, 0);
}
...
function worldCosts(level = worldLevel()) {
  const activation = level >= 10 ? 20 : level >= 5 ? 10 + Math.max(0, level - 5) * 2 : 5 + Math.max(0, level - 1);
  const maintenance = level >= 10 ? 6 : level >= 5 ? 4 + Math.floor(Math.max(0, level - 5) / 2) : 2 + Math.floor(Math.max(0, level - 1) / 2);
...
function caBreakdown() {
  try {
    ...
    const total = base + armor + effects + talents;
    ...
    return { total, base, armor, effects, talents, block };
  } catch (error) {
    addError("LAT-CALC-001", error.message);
    return { total: 0, base: 0, armor: 0, effects: 0, talents: 0, block: { cortante: 0, perfurante: 0, contundente: 0 } };
  }
}
...
function renderResumo() {
  ...
  return `
    ...
    <section class="panel">
      <div class="section-title"><h2>Atributos</h2><span class="muted small">Clique no nome do atributo para ver sucessos.</span></div>
      <div class="grid four">
    ...
    <div class="summary-layout">
      ...
      <aside class="panel quick-skills-side">
...
function renderCombate() {
  ...
  return `
    <section class="panel">
      <div class="section-title"><h2>Combate</h2><button class="button" type="button" data-action="reset-actions">Nova rodada</button></div>
      <div class="grid two">
        <div class="stack">
        ...
        <div class="card stack">
          ...
          <div class="stat-grid">
            ...
            ${miniStat("CA", ca.total)}
            ${miniStat("Bloqueio", `C ${ca.block.cortante} · P ${ca.block.perfurante} · Cn ${ca.block.contundente}`)}
...
function renderInventario() {
  ...
  return `
    <section class="panel">
      ...
      <div class="grid four">
        ${field("Dinheiro X", "inventory.money.X", "number")}
        ${field("Dinheiro D", "inventory.money.D", "number")}
        ${field("Dinheiro L", "inventory.money.L", "number")}
        ${selectField("Armadura", "inventory.armorId", armorOptions, "Sem armadura")}
        <label class="field">Escudo<input type="checkbox" data-path="inventory.shield" ${state.inventory.shield ? "checked" : ""}></label>
      </div>
    </section>
    <section class="panel">
    ...
    <section class="panel">
      <div class="section-title"><h2>Equipamentos</h2><div class="inline"><button class="ghost" type="button" data-action="open-equipment-picker">Equipamento</button><button class="ghost" type="button" data-action="open-custom-equipment">Equipamento customizável</button></div></div>
...
function spellLevelCard(type) {
  ...
  const next = level < 10 ? aptitudeCost(type, level + 1) : 0;
...
function renderLevels(spell, activeLevel = 0) {
  ...
  return `<ol class="level-list">${spell.levels.map((level) => `<li><strong>N${level.level}</strong> ${level.level <= activeLevel ? `<span class="tag ok">ativo</span>` : ""}<br>${esc(level.text)}</li>`).join("")}</ol>`;
}

function extraSpellCard(known) {
  const spell = getSpell(known.type, known.regionCode);
  const next = known.level < 10 ? aptitudeCost(known.type, known.level + 1) : 0;
  return `<div class="spell-card">
...
function spellCombatCard(item) {
  return `<div class="spell-card">
    ...
    <button class="button" type="button" data-action="activate-magic" data-type="${esc(item.type)}">${item.type === "Mundo" ? "Abrir/usar Mundo" : "Ativar magia"}</button>
  </div>`;
}
...
function handleKeydown(event) {
  ...
  if (event.altKey && event.key.toLowerCase() === "r") {
    ...
    state.combat.log.unshift("Nova rodada.");
    render();
    event.preventDefault();
  }
}
...
function openCaModal() {
  ...
  openModal("Classe de Armadura", `
    <div class="grid two">
      <div class="card">
        <h3>Cálculo</h3>
        <p>20 + (DES ÷ 5) - 30 + armadura + escudo + efeitos + talentos</p>
        <ul>
...
function handleClick(event) {
  ...
  const actions = {
    ...
    "start-new": () => { state.meta.started = true; closeModal(); render(); },
    ...
    "activate-magic": () => activateMagic(button.dataset.type),
    "adjust-resource": () => adjustResource(button.dataset.resource, num(button.dataset.delta, 0)),
    "reset-actions": () => { state.combat.actions = { standard: true, bonus: true, movement: true, reaction: true }; state.combat.log.unshift("Nova rodada."); render(); },
    ...
    "open-equipment-picker": openEquipmentPicker,
    "open-custom-equipment": () => openCustomEquipment(),
    "add-equipment": () => addEquipmentFromModal(button.dataset.name),
...
function changeMagicLevel(type, delta) {
  ...
  if (delta > 0 && aptitudeTotal() - aptitudeSpent() < aptitudeCost(type, next)) return addError("LAT-MAG-001", `${type} N${next}`);
...
function openWeaponPicker(kind) {
  ...
  openModal(kind === "cultural" ? "Arma cultural" : "Armas", `<div class="table-wrap"><table><thead><tr><th>Nome</th><th>Dano</th><th>Peso</th><th>Propriedade</th><th></th></tr></thead><tbody>${weapons.map((weapon, index) => `<tr><td>${esc(weapon.name)}</td><td>${esc(weapon.damage)}</td><td>${esc(weapon.weight)}</td><td>${esc(weapon.property)}</td><td><button class="button" type="button" data-action="add-weapon" data-index="${index}">Adicionar</button></td></tr>`).join("")}</tbody></table></div>`);
}
...
function openEquipmentPicker() {
  openModal("Equipamentos", `<div class="skill-list">${DB.equipment.map((name) => `<div class="skill-pill"><strong>${esc(name)}</strong><button class="button" type="button" data-action="add-equipment" data-name="${esc(name)}">Adicionar</button></div>`).join("")}</div>`);
}

function addEquipmentFromModal(name) {
  state.inventory.equipment.push({ id: uid(), name, category: "Equipamento", qty: 1, weight: "", description: "" });
  closeModal();
  render();
}
...
async function importPdf(file) {
  try {
    ...
    if (!/PERSONAGEM|ATRIBUTOS|PERÍCIAS|PERICIAS/i.test(text)) {
      addError("LAT-PDF-004");
      return;
    }
    const { data, missing } = parsePdfText(text);
    applyImportedPdfData(data);
    state.meta.started = true;
    closeModal();
    render();
    if (missing.length) showMissingPdfFields(missing);
