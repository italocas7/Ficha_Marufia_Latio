...
const validTauriManifest = Object.freeze({
  ...
  platforms: Object.freeze({
    "windows-x86_64": Object.freeze({
      url: "https://github.com/italocas7/Ficha_Marufia_Latio/releases/download/v0.3.1/Marufia-Setup.exe",
...
test("shows the confirmation only for a valid newer native update", async () => {
  for (const [version, expected] of [["0.3.2", true], ["0.3.1", false], ["0.3.0", false], ["inválida", false]]) {
    const resource = fakeUpdate({ version });
    const environment = fakeEnvironment({ updates: [resource.update] });
    const checker = updates.createUpdateChecker({ document: environment.document, view: environment.view, versionInfo: { version: "0.3.1" } });
    ...
    if (expected) {
      ...
      assert.match(environment.modal.body, /Instalada: v0\.3\.1/);
      assert.match(environment.modal.body, /Nova: v0\.3\.2/);
...
test("Agora não suppresses one version only for the current application session", async () => {
  ...
  assert.equal(environment.storage.get(updates.DISMISS_KEY), "0.3.2");
  assert.equal(environment.modal.open, false);
  assert.equal(resource.state.closed, 1);

  const sameSession = fakeEnvironment({ storage: environment.storage });
  const sameChecker = updates.createUpdateChecker({ document: sameSession.document, view: sameSession.view, versionInfo: { version: "0.3.1" } });
  assert.equal(await sameChecker.check(), false);
  assert.equal(sameSession.modal.open, false);

  const freshSession = fakeEnvironment();
  const freshChecker = updates.createUpdateChecker({ document: freshSession.document, view: freshSession.view, versionInfo: { version: "0.3.1" } });
...
test("offers retry and the scoped manual release after an installation failure", async () => {
  ...
  assert.deepEqual(environment.opened, ["https://github.com/italocas7/Ficha_Marufia_Latio/releases/tag/v0.3.2"]);
  assert.equal(environment.modal.open, false);
});
...
test("escapes notes and ships both prepared 0.3.1 manifests", () => {
  const html = updates.updateBodyHtml({ version: "0.3.2", notes: "<img src=x onerror=alert(1)>" }, "0.3.1");
  ...
  assert.equal(legacy.version, "0.3.1");
  assert.equal(signed.version, "0.3.1");
