...
test("serves static assets first and falls back to the sheet entry page", () => {
  ...
  assert.match(worker, /TAURI_UPDATE_MANIFEST_PATH = "\/tauri-update\.json"/);
  assert.match(worker, /TAURI_UPDATE_MANIFEST_ASSET_PATH = "\/\.marufia\/tauri-update\.json"/);
...
test("serves both update manifests cross-origin without browser or edge caching", async () => {
  ...
  for (const manifestName of ["app-update.json", "tauri-update.json"]) {
    const response = await worker.fetch(
      new Request(`https://example.test/${manifestName}?check=1`),
      {
        ASSETS: {
          fetch: async (request) => {
            assert.equal(new URL(request.url).pathname, `/.marufia/${manifestName}`);
...
test("applies the update manifest headers when the static asset is served before the Worker", () => {
  ...
  assert.match(headers, /^\/tauri-update\.json$/m);
...
test("keeps public manifest paths out of static assets so the Worker controls their headers", () => {
  const build = fs.readFileSync(path.join(root, "tools", "build_site.py"), "utf8");
  assert.match(build, /"app-update\.json": "\.marufia\/app-update\.json"/);
  assert.match(build, /"tauri-update\.json": "\.marufia\/tauri-update\.json"/);
  assert.match(build, /relative in UPDATE_MANIFEST_ASSETS/);
  assert.match(fs.readFileSync(path.join(root, "tools", "test_site_package.cjs"), "utf8"), /\["app-update\.json", "tauri-update\.json"\]/);
});
...
test("packages the strict desktop update manifest without changing sheet data", () => {
  ...
  assert.equal(signed.platforms["windows-x86_64"].url, `https://github.com/italocas7/Ficha_Marufia_Latio/releases/download/v${manifest.version}/Marufia-Setup.exe`);
  const build = fs.readFileSync(path.join(root, "tools", "build.py"), "utf8");
  const index = fs.readFileSync(path.join(root, "index.html"), "utf8");
  assert.match(build, /"app-update\.json"/);
  assert.match(build, /"tauri-update\.json"/);
