...
`Marufia.exe` e `Marufia-Setup.exe` foram verificados pelo Windows como `NotSigned`. Nenhum certificado comercial ou autoassinado foi adquirido, criado ou instalado. Por isso, o Alpha privado pode mostrar **Editor desconhecido** ou **O Windows protegeu o computador**. Isso é esperado para esta distribuição, não altera a ficha e não impede o teste privado em computadores cuja política permita continuar.
...
## Procedimento seguro para o Alpha privado
